...
def _create_annotations(gt, camera_id, frame_shape):
    ...
    default_frame_shape = (1080, 1920, 3)
    player_delta = -6
    ball_delta = -8
    frame_resize_factor = (
        frame_shape[0] / default_frame_shape[0],
        frame_shape[1] / default_frame_shape[1]
    )

    for (start_frame, end_frame, x, y) in gt['BallPos']:
        for i in range(start_frame, end_frame+1):
            y = round(y * frame_resize_factor[1])
            x = round(x * frame_resize_factor[0])

            if camera_id == 2 or camera_id == 6:
                # For some reason ball coordinates for camera 2 and 6 have reversed in x-coordinate
                x = frame_shape[1] - x
            annotations.ball_pos[i+ball_delta].append((x, y))

    for (start_frame, end_frame, value) in gt['BallShot']:
        if value:
            for i in range(start_frame, end_frame+1):
                annotations.ball_shot[i+ball_delta] = True

    for (start_frame, end_frame, value) in gt['PlayerInteractingID']:
        if value > -1:
            for i in range(start_frame, end_frame+1):
                annotations.interacting_player[i+player_delta].append(value)

    for player in gt['Person']:
        for (start_frame, end_frame, height, width, x, y) in gt['Person'][player]:
            assert start_frame <= end_frame
            for i in range(start_frame, end_frame+1):
                height = round(height * frame_resize_factor[1])
                width = round(width * frame_resize_factor[0])
                x = round(x * frame_resize_factor[1])
                y = round(y * frame_resize_factor[0])
                annotations.persons[i+player_delta].append((player, height, width, x, y))
...
def read_issia_ground_truth(camera_id, dataset_path, frame_shape):
    '''
    ...
    annotations = _create_annotations(gt, camera_id, frame_shape)

    return annotations


def evaluate_ball_detection_results(detections, ground_truths, tolerance):
    '''
    Evaluate ball detection performance
    :param detections: SequenceAnnotations object with ball detection results to be evaluated
    :param ground_truths: SequenceAnnotations object with ground truth annotations
    ...
    for i in detections:
        ball_pos = [det.centroid for det in detections[i] if det.score >= .7]
        gt_ball_pos = [gt.centroid for gt in ground_truths[i]]
...
if __name__ == '__main__':
    ...
    precision, recall, correctly_classified = _ball_detection_stats(det, gt, tolerance)
    print('gr')
